...
function mockRes() {
  const res = {
    ...
    end(data) { res.body = data ? JSON.parse(data) : null; return res },
  }
  return res
}
...
function mockDb(data = {}) {
  return {
    ...
    select(table) { return this.data[table] || [] },
...
describe('create controller', () => {
  it('should create a ticket with defaults', () => {
    const res = mockRes()
    const db = mockDb()
    create({ req: { body: { equipment: 'PC', description: 'broken', user_name: 'John' } }, res, database: db })
  ...
  it('should reject invalid priority', () => {
    const res = mockRes()
    create({ req: { body: { equipment: 'PC', description: 'd', user_name: 'J', priority: 'INVALID' } }, res, database: mockDb() })
  ...
  it('should accept all valid priorities', () => {
    for (const p of ['LOW', 'MEDIUM', 'HIGH', 'CRITICAL']) {
      const res = mockRes()
      create({ req: { body: { equipment: 'X', description: 'd', user_name: 'J', priority: p } }, res, database: mockDb() })
      expect(res.statusCode).toBe(201)
  ...
  it('should normalize lowercase priority', () => {
    const res = mockRes()
    create({ req: { body: { equipment: 'X', description: 'd', user_name: 'J', priority: 'high' } }, res, database: mockDb() })
    expect(res.statusCode).toBe(201)
...
describe('index controller', () => {
  it('should list all tickets', () => {
    const res = mockRes()
    const db = mockDb({ tickets: [{ id: '1', equipment: 'A' }, { id: '2', equipment: 'B' }] })
...
describe('update controller', () => {
  it('should update a ticket', () => {
    const res = mockRes()
    const db = mockDb({ tickets: [{ id: '1', equipment: 'A', description: 'd' }] })
    update({ req: { params: { id: '1' }, body: { equipment: 'B' } }, res, database: db })
...
describe('updateStatus controller', () => {
  it('should close a ticket', () => {
    ...
    updateStatus({ req: { params: { id: '1' }, body: { solution: 'fixed' } }, res, database: db })
